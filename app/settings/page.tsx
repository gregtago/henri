      ...
      <div className="flex-1 flex flex-col min-h-0">
        ...
        <div className={"max-w-4xl mx-auto px-6 pt-8 pb-[104px] md:pb-8 space-y-6"}>
          ...
          {/* ── ACCUEIL ──
            ...
            *
            * Le fond passe à l'encre et la carte au clair : la seule page de
            * l'application qui s'inverse, parce qu'elle ne se travaille pas —
            * on la reconnaît avant de l'avoir lue. La nuit, l'encre et le clair
            * échangent leurs rôles d'eux-mêmes : ce sont les mêmes deux jetons
            * (`--text` et `--bg`), qui se retournent avec le thème. */}
          {tab === "accueil" && (
            <div className="rounded-[20px] p-4 sm:p-6" style={{ background: "var(--text)" }}>

              <div className="flex flex-col items-center gap-2 pt-5 pb-6">
                <img src="/logo-henri-new.png" alt="Henri" className="logo-on-ink" style={{ height: "44px", width: "auto" }} />
                <p className="text-[11.5px] text-tx-3">{VERSIONS[0].v}</p>
              </div>

              {!user ? (
                <div className="bg-bg rounded-[14px] p-5 text-[13px] text-tx-2">Connectez-vous pour gérer votre compte.</div>
              ) : (
                <div className="bg-bg rounded-[14px] px-5">
