...
// Les notes de version — la donnée, plutôt qu'une liste noyée dans le rendu :
...
export default function SettingsPage() {
  ...
  // On ouvre sur l'accueil : le rail range le titre courant au bord gauche, et
  ...
  // Le « Enregistré ✓ » de l'Apparence : un réglage chassé par un autre ne doit
  // pas voir sa confirmation éteinte par le minuteur du précédent.
  ...
  // « Préférences → Sécurité » se cite dans les écrans, les courriels et la
  // proposition de second facteur : `?tab=securite` y mène directement, plutôt
  // que de déposer le lecteur sur la page d'accueil à charge pour lui de
  // retrouver le bon titre. Un nom inconnu est ignoré — une URL bricolée ne
  // doit pas ouvrir un onglet qui n'existe pas.
  useEffect(() => {
    const asked = new URLSearchParams(window.location.search).get("tab");
    if (asked && (TABS as string[]).includes(asked)) setTab(asked as Tab);
  }, []);
  ...
  /**
   * Un réglage d'apparence se pose, il ne se signe pas.
   *
   * Le choix s'appliquait déjà à l'écran dès le clic, mais n'était écrit
   * qu'après un bouton « Enregistrer » que seul cet onglet portait : partout
   * ailleurs — Rappels, Raccourci, Modèles — le geste vaut enregistrement.
   * Qui changeait de police et repartait voyait donc l'interface changer, puis
   * l'oublier au rechargement. On écrit maintenant à chaque geste, et le
   * « Enregistré ✓ » ne fait que le dire.
   */
      ...
      <div className="flex-1 flex flex-col min-h-0">
        ...
        <div className={"max-w-4xl mx-auto px-6 pt-8 pb-[112px] md:pb-8 space-y-6"}>
          ...
          {/* ── ACCUEIL ──
            ...
            * endroit de l'application, avec l'écran d'attente et la connexion,
            * où le logo a encore quelque chose à faire. Dessous, ce que le rond
            * du compte tenait — qui l'on est, les notifications de cet appareil,
            * l'installation, la sortie. Et c'est tout. */}
          {tab === "accueil" && (
                ...
                <div className="bg-bg border border-border rounded-xl px-5">
                  ...
                  <div className={row}>
                    <div className="flex items-center gap-3 min-w-0">
                      <span className="w-9 h-9 shrink-0 rounded-full border border-border bg-bg-subtle text-tx-2 flex items-center justify-center text-[14px] font-medium">
                        {user.email?.trim()?.[0]?.toUpperCase() ?? ""}
                      </span>
                      <div className="min-w-0">
                        <p className={`${lbl} break-all`}>{user.email}</p>
                        <p className={sublbl}>Connecté sur cet appareil.</p>
                      </div>
                    </div>
                  </div>
                  ...
                  {deviceReminders.status !== "unsupported" && (
                    <div className={row}>
                      <div className="min-w-0">
                        <p className={lbl}>Notifications sur cet appareil</p>
                        <p className={sublbl}>
            ...
            <div className="space-y-4">
              ...
              <section>
                <SectionTitle help={{
                  ...
                  body: <>
                    <p>À la connexion, Henri demande un <strong className="text-tx">code à six chiffres</strong> lu sur votre téléphone, en plus du mot de passe. Le code vient d&apos;une application d&apos;authentification : <AuthAppLinks />.</p>
            ...
            <div className="space-y-4">
              ...
              {!user ? (
                ...
                <div className="bg-bg border border-border rounded-xl p-5 text-[13px] text-tx-2">
                  Aucun appareil enregistré pour l'instant. Sur l'appareil à ajouter, ouvrez Préférences → Accueil, puis « Activer ici ».
                </div>
              ) : (
