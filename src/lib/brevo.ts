// Les courriels d'Henri — invitation, mot de passe, adresse.
//
// Tous partent de la même adresse (`SENDER`) et du même gabarit : ce sont des
// courriels de l'Office, pas des courriels de Firebase. Le SDK admin sait
// fabriquer les liens d'action sans rien envoyer (`generatePasswordResetLink`,
// `generateEmailVerificationLink`) ; c'est ce qui permet de garder la main sur
// la forme, et de n'avoir qu'un seul expéditeur à faire authentifier
// (SPF/DKIM sur mail.tagot.fr) plutôt que deux.
...
<tr><td style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,0.08);">
  ...
  <tr><td style="padding:44px 52px 40px;">
    <img src="https://i.imgur.com/MKLZSiT.jpeg" alt="Henri" width="80" style="display:block;margin-bottom:36px;height:auto;" />
    <h1 style="margin:0 0 24px;font-size:24px;font-weight:700;color:#111827;line-height:1.3;">Confirmez votre adresse</h1>
    <p style="margin:0 0 14px;font-size:15px;color:#4b5563;line-height:1.8;">Un clic suffit. C'est ce qui nous assure que cette adresse est bien la vôtre — et c'est nécessaire avant d'activer la double authentification.</p>
    <p style="margin:0 0 36px;font-size:15px;color:#4b5563;line-height:1.8;">Si vous n'êtes pas à l'origine de cette demande, ignorez cet email.</p>
    <table cellpadding="0" cellspacing="0">
      <tr><td style="background:#111827;border-radius:8px;">
        <a href="${link}" style="display:inline-block;padding:15px 36px;font-size:15px;font-weight:600;color:#ffffff;text-decoration:none;">Confirmer mon adresse &rarr;</a>
      </td></tr>
    </table>
    <p style="margin:36px 0 0;font-size:11px;color:#9ca3af;line-height:1.8;">
      Si le bouton ne fonctionne pas, copiez ce lien :<br>
  ...
  const text = `Confirmez votre adresse — Henri\n\n${link}\n\nGrégoire TAGOT`;
  await sendBrevoEmail(email, "Confirmez votre adresse — Henri", html, text);
}

export async function sendInvitationEmail(email: string, name: string | null, link: string) {
  const html = `<!DOCTYPE html>
<html lang="fr">
...
Grégoire TAGOT · 2 rue Dante, 75005 Paris · gregoire@tagot.fr`;
  ...
  await sendBrevoEmail(email, "Votre invitation à rejoindre Henri", html, text);
}
